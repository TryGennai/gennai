...
public class SubmitTopologyHook extends BaseCommandHook {

  private static final Pattern STATEMENT_PATTERN = Pattern
      .compile("^SUBMIT\\s+TOPOLOGY\\s+\\w+", Pattern.CASE_INSENSITIVE);
    ...
    try {
      try {
        ...
        for (; cnt < times; cnt++) {
          ...
          String res = getContext().getStatement().execute("DESC TOPOLOGY");
