...
public class CreateUserTask implements Task {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(CreateUserTask.class);

  private UserEntity user;
  private UserEntity owner;

  public CreateUserTask(UserEntity user, UserEntity owner) {
    this.user = user;
    this.owner = owner;
  }
  ...
  public String execute() throws TaskExecuteException {
    if (owner.getName().equals(ROOT_USER_NAME)) {
      try {
        GungnirManager.getManager().getMetaStore().insertUserAccount(user);
