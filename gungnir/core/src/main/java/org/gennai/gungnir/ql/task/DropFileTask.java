...
public class DropFileTask implements Task {

  private String fileName;
  private UserEntity owner;

  public DropFileTask(String fileName, UserEntity owner) {
  ...
  public String execute() throws TaskExecuteException {
    try {
      ...
      FileStat fileStat = metaStore.findFile(fileName, owner);
      if (!fileStat.getTopologies().isEmpty()) {
        throw new TaskExecuteException("'" + fileName + "' has been used");
      }

      if (!metaStore.deleteFile(fileStat)) {
        throw new TaskExecuteException("'" + fileName + "' has been locked");
      }
