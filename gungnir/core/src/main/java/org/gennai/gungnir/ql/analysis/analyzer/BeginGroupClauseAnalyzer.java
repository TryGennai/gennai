...
public class BeginGroupClauseAnalyzer {

  private SemanticAnalyzer semanticAnalyzer;

  public BeginGroupClauseAnalyzer(SemanticAnalyzer semanticAnalyzer) {
  ...
  public Stream analyze(ASTNode node, Stream stream)
    ...
    if (stream instanceof SingleStream) {
      return ((SingleStream) stream).beginGroupBy(fields);
    } else {
      return ((GroupedStream<?>) stream).beginGroupBy(fields);
    }
  }
}
