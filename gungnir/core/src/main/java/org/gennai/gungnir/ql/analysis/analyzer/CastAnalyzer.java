...
public class CastAnalyzer implements Analyzer<Function<?>> {

  @Override
  public Function<?> analyze(ASTNode node, SemanticAnalyzer semanticAnalyzer)
      throws SemanticAnalyzeException {
    Object castExpr = semanticAnalyzer.analyzeByAnalyzer(node.getChild(0));
    FieldType fieldType = semanticAnalyzer.analyzeByAnalyzer(node.getChild(1));

    Cast func = null;
    try {
      func = new Cast().create(castExpr, fieldType);
    } catch (Exception e) {
      throw new SemanticAnalyzeException("Faild to create 'cast' function", e);
    }
