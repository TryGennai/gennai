...
public class FunctionAnalyzer implements Analyzer<Function<?>> {

  @Override
  public Function<?> analyze(ASTNode node, SemanticAnalyzer semanticAnalyzer)
      throws SemanticAnalyzeException {
    String functionName = semanticAnalyzer.analyzeByAnalyzer(node.getChild(0));
    Object[] args = semanticAnalyzer.analyzeByAnalyzer(node.getChild(1));

    Function<?> function = null;
    try {
      function = semanticAnalyzer.getFunctionRegistry().create(functionName, args);
    } catch (SemanticAnalyzeException e) {
      throw e;
    } catch (Exception e) {
      throw new SemanticAnalyzeException("Failed to create '" + functionName + "' function", e);
    }
    return function;
  }
}
