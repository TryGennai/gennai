...
public class MapAnalyzer implements Analyzer<Map<Object, Object>> {

  @Override
  public Map<Object, Object> analyze(ASTNode node, SemanticAnalyzer semanticAnalyzer)
      throws SemanticAnalyzeException {
    Map<Object, Object> map = Maps.newLinkedHashMap();
    for (int i = 0; i < node.getChildCount(); i += 2) {
      map.put(semanticAnalyzer.analyzeByAnalyzer(node.getChild(i)),
          semanticAnalyzer.analyzeByAnalyzer(node.getChild(i + 1)));
    }
    return map;
  }
}
