...
public class FilterClauseAnalyzer {

  private SemanticAnalyzer semanticAnalyzer;

  public FilterClauseAnalyzer(SemanticAnalyzer semanticAnalyzer) {
    this.semanticAnalyzer = semanticAnalyzer;
  }

  public Stream analyze(ASTNode node, Stream stream)
      throws SemanticAnalyzeException, GungnirTopologyException {
    Condition condition = semanticAnalyzer.analyzeByAnalyzer(node.getChild(0));
    Integer parallelism = semanticAnalyzer.analyzeByAnalyzer(node.getChild(2));

    if (stream instanceof SingleStream) {
      return ((SingleStream) stream).filter(condition).parallelism(parallelism);
    } else {
      return ((GroupedStream<?>) stream).filter(condition).parallelism(parallelism);
    }
  }
}
