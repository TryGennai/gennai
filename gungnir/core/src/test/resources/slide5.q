...
BEGIN GROUP BY aaa
SLIDE LENGTH 30sec BY _time aaa, sum(DISTINCT bbb) AS s, count(DISTINCT ddd) AS c, avg(DISTINCT bbb) AS a
...
@IS('Explain:
 ...
 PARTITION_1(fields grouping(aaa))
  -GS[aaa]-> SLIDE_2
 SLIDE_2(length(30SECONDS BY _time), [aaa, sum(distinct(bbb)) AS s, count(distinct(ddd)) AS c, avg(distinct(bbb)) AS a]) parallelism=2
  -GS[aaa]-> EMIT_3
...
@EMIT('EMIT_3', '{aaa:"aaa1", s:10, c:2, a:10}');
@EMIT('EMIT_3', '{aaa:"aaa2", s:10, c:1, a:10}');
@EMIT('EMIT_3', '{aaa:"aaa2", s:10, c:2, a:10}');
@EMIT('EMIT_3', '{aaa:"aaa3", s:10, c:1, a:10}');
@EMIT('EMIT_3', '{aaa:"aaa3", s:10, c:2, a:10}');
@PLAY(60);

@POST('tuple1', '{aaa:"aaa1", bbb:50, ccc:100, ddd:"ddd1"}');
@POST('tuple1', '{aaa:"aaa3", bbb:60, ccc:100, ddd:"ddd3"}');
@EMIT('EMIT_3', '{aaa:"aaa1", s:60, c:2, a:30}');
@EMIT('EMIT_3', '{aaa:"aaa3", s:70, c:2, a:35}');
@PLAY(60);
...
@EMIT('EMIT_3', '{aaa:"aaa1", s:120, c:3, a:40}');
@EMIT('EMIT_3', '{aaa:"aaa3", s:90, c:2, a:30}');
@PLAY(60);
...
@EMIT('EMIT_3', '{aaa:"aaa10", s:10, c:2, a:10}');
@EMIT('EMIT_3', '{aaa:"aaa11", s:10, c:1, a:10}');
@PLAY(60);

STOP TOPOLOGY slide5;
